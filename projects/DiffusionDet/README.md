...
3. Testing the model in MMDetection.
   ...
   ```shell
   python tools/test.py projects/DiffusionDet/configs/diffusiondet_config.py ${CHECKPOINT_PATH}
   ```
...
```bash
python tools/train.py projects/DiffusionDet/configs/diffusiondet_config.py
```

For multi-gpu training, run:

```bash
python -m torch.distributed.launch --nnodes=1 --node_rank=0 --nproc_per_node=${NUM_GPUS} --master_port=29506 --master_addr="127.0.0.1" tools/train.py projects/DiffusionDet/configs/diffusiondet_config.py
```
...
# test command
python tools/test.py projects/DiffusionDet/configs/diffusiondet_config.py ${CHECKPOINT_PATH} --cfg-options model.bbox_head.sampling_timesteps=4
```
